...
def test_read_stdout(capfd,monkeypatch):
    ...
    assert captured[0].strip() == host_address_1_network_address_string, "Het gevraagde netwerkadres klopt niet. Je moet een bitwise and van het netmasker en het hostadres doen."
    assert captured[1].strip() == host_address_2_binary_string, "Het gevraagde hostadres klopt niet. Je moet een bitwise or van het netwerkadres en het hostgedeelte doen."
